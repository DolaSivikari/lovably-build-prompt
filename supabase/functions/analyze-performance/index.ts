...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const [metricName, threshold] of Object.entries(thresholds)) {
      ...
      const rating =
        avgValue <= threshold.good
          ? "good"
          : avgValue <= threshold.needsImprovement
            ? "needs-improvement"
            : "poor";

      console.log(
        `[Analyze Performance] ${metricName}: ${avgValue.toFixed(2)} (${rating})`,
      );
    ...
    console.log(
      `[Analyze Performance] Generated ${recommendations.length} recommendations`,
    );
