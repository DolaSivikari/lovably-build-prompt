...
export const ProjectTimelineEstimator = () => {
  ...
  const calculateTimeline = () => {
    ...
    const complexityAdjusted =
      baseTime * (complexityMultipliers[complexity] || 1);

    // Add buffers for real-world factors
    const weatherBuffer = weatherSensitive ? complexityAdjusted * 0.2 : 0; // 20% for weather
    const contingencyBuffer = complexityAdjusted * 0.15; // 15% general contingency

    return {
      minimum: Math.ceil(complexityAdjusted * 0.8), // Best case (20% faster)
      typical: Math.ceil(complexityAdjusted), // Expected timeline
      maximum: Math.ceil(
        complexityAdjusted + weatherBuffer + contingencyBuffer,
      ), // With delays
        ...
        <div className="space-y-4 pt-4 border-t">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between p-2 bg-muted/50 rounded">
              ...
              <span className="font-semibold">
                {timeline.productionRate.toLocaleString()} sq ft/day
              </span>
            </div>
            {weatherSensitive && (
          ...
          <div className="p-4 bg-primary/10 rounded-lg border-2 border-primary/20">
            <p className="text-sm font-medium text-center">
              ...
              <strong>{timeline.typical} working days</strong> for this project
            </p>
          </div>
        </div>
      </CardContent>
